...
class PoolGroup (models.Model, HelperMixins):
    ...
    @classmethod
    def get_superuser_index_view_model(cls, site_user, modelstate, poolgroups, filter = None,
         groupowner_id = None):

        modelstate, modelstate_success = PoolGroup.get_modelstate(modelstate)
        ...
        if filter == None or filter == 0:
            ...
            if groupowner_id != None:
                poolgroups = PoolGroup.filter_poolgroups(groupowner_id)

        view_model = {'title': 'Pool Group Index', # app/layout.html params
                        'site_user':site_user, 
                        'year': datetime.now().year,

                        'partial_view_id': 'poolgroup-id', # app/shared_index.html params
                        'navigation': 'app/user_navigation.html',
                        'model_groupowner_id': groupowner_id,
                        'filter' : filter,
                        'modelstate': modelstate, 
                        'modelstate_success': modelstate_success, 
                        'create_item_url': 'poolgroup:poolgroup_create', 
                        'create_item_name': 'Create Pool Group', 
                        'index_table': 'poolgroup/index_table.html', 
                        'scripts': ['app/scripts/Client/TableStripping.js'],

                        'superuser_filter': 'poolgroup/superuser_filter.html', #app/user_navigation.html
                        'groupowner_filter': 'poolgroup/groupowner_filter.html',
                        
                        'model_groupowners': groupowners, #poolgroup/groupowner_pagination.html
                        'index_url': 'poolgroup:poolgroup_index',
 
                        'items': poolgroups, # poolgroup/index_table.html params
                        'item_label': 'Pool Group Name', 
                        'item_url': 'poolowner:poolowner_index',
                        'details_url': 'poolgroup:poolgroup_details', 
                        'delete_url': 'poolgroup:poolgroup_delete', 
                        } 
        
        return view_model

    @classmethod
    def get_groupowner_index_view_model(cls, site_user, modelstate, poolgroups):
